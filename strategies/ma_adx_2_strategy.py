    ...
    def on_bar(self, bar: BarEvent) -> SignalEvent:
        ...
        if self.position != 0:
            ...
            if pnl <= -self.stop_loss:
                ...
                sig.timestamp = bar.timestamp # 👈 給單子蓋上時間戳記
                return sig
        ...
        # 🚀 空頭：價格必須小於快線 (證明反彈結束，再次破底)
            ...
            # ==========================================
            # 🛡️ 空軍開關判定：允許做空 vs 純多頭撤退
            # ==========================================
            if self.enable_short and self.position >= 0:
                # 傳統模式：平多單並反手做空
                signal = SignalEvent(
            ...
            elif not self.enable_short and self.position > 0:
                # 🚀 純多頭模式 (Long-Only)：遇到死叉，只獲利了結平倉，不准做空！
                signal = SignalEvent(
                    type=EventType.SIGNAL, symbol=bar.symbol, signal_type=SignalType.FLATTEN,
                    reason=" | ".join(reason_parts) + " (純多頭避險平倉)"
                )
    ...
    def get_ui_dict(self):
        ...
        if self.position != 0 and hasattr(self, 'entry_price') and self.entry_price > 0:
            ...
            if self.position > 0:
                ...
                # 🚀 修正：用「曾經達到的最大獲利」來判斷是否解鎖，而不是用當前 pnl
                max_unrealized_pnl = high_p - self.entry_price
                
                if self.enable_trailing_stop and max_unrealized_pnl >= self.trailing_trigger:
                ...
                max_unrealized_pnl = self.entry_price - low_p
